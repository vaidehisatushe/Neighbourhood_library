...
app.post('/books', (req, res) => client.CreateBook({ book: req.body }, (err, response) => err ? res.status(500).json(err) : res.json(response.book)));
...
app.post('/members', (req, res) => client.CreateMember({ member: req.body }, (err, response) => err ? res.status(500).json(err) : res.json(response.member)));
...
app.delete('/members/:id', (req, res) => {
  const id = Number(req.params.id);
  client.DeleteMember ? client.DeleteMember({ id }, (err, response) => err ? res.status(500).json(err) : res.json(response)) : res.status(501).json({ error: 'DeleteMember not implemented on server' });
});
